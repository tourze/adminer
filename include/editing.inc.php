 ...
 */
function select($result, $connection2 = null, $orgtables = [], $limit = 0, & $dataList = [], $print = true)
{
    global $jush;
    ...
    for ($i = 0; ( ! $limit || $i < $limit) && ($row = $result->fetch_row()); $i++)
    {
        if ( ! $i)
        {
            echo $print ? "<table cellspacing='0' class='nowrap'>\n" : '';
            echo $print ? "<thead><tr>" : '';
            for ($j = 0; $j < count($row); $j++)
            {
                ...
                $types[$j] = $field->type;
                $dataList['fields'][] = $field->name;
                echo $print ? "<th" . ($orgtable != "" || $field->name != $orgname ? " title='" . h(($orgtable != "" ? "$orgtable." : "") . $orgname) . "'" : "") . ">" . h($name)
                    . ($orgtables ? doc_link(['sql' => "explain-output.html#explain_" . strtolower($name)]) : "") : '';
            }
            echo $print ? "</thead>\n" : '';
        }
        echo $print ? "<tr" . odd() . ">" : '';
        ...
        $dataList['records'][] = $one;
    }
    echo $print ? ($i ? "</table>" : "<p class='message'>" . lang('No rows.')) . "\n" : '';
    return $return;
}
 ...
 * @return null
 */
function edit_type($key, $field, $collations, $foreign_keys = [])
{
    global $structured_types, $types, $unsigned, $on_actions;
    $type = $field["type"];
    ?>
    <td><select name="<?php echo $key; ?>[type]" class="type" onfocus="lastType = selectValue(this);"
    ...
    echo(isset($field['on_update']) ? "<select name='$key" . "[on_update]'" . (preg_match('~timestamp|datetime~', $type) ? "" : " class='hidden'") . '>' . optionlist(["" => "(" . lang('ON UPDATE') . ")",
 ...
 * @return null
 */
function edit_fields($fields, $collations, $type = "TABLE", $foreign_keys = [], $comments = false)
{
    ...
    <tr class="wrap">
        ...
        <td><input type="radio" name="auto_increment_col" value=""><acronym
                title="<?php echo lang('Auto Increment'); ?>">AI</acronym><?php echo doc_link([
        ...
        <td><?php echo lang('Default value'); ?>
            ...
            <?php } ?>
        <td><?php echo "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($fields)) . "]' src='static/plus.gif' alt='+' title='" . lang('Add next') . "'>"; ?>
    ...
    <tr<?php echo($display ? "" : " style='display: none;'"); ?>>
        ...
        <th><?php if ($display)
    { ?><input name="fields[<?php echo $i; ?>][field]" value="<?php echo h($field["field"]); ?>"
    ...
    { ?>
        ...
        <td><label class="block"><input type="radio" name="auto_increment_col"
                                        value="<?php echo $i; ?>"<?php if ($field["auto_increment"])
                { ?> checked<?php } ?>
    ...
    <?php } ?>
        ...
        echo(support("move_col") ?
            "<input type='image' class='icon' name='add[$i]' src='static/plus.gif' alt='+' title='" . lang('Add next') . "' onclick='return !editingAddRow(this, 1);'>&nbsp;"
            . "<input type='image' class='icon' name='up[$i]' src='static/up.gif' alt='^' title='" . lang('Move up') . "'>&nbsp;"
            . "<input type='image' class='icon' name='down[$i]' src='static/down.gif' alt='v' title='" . lang('Move down') . "'>&nbsp;"
            : "");
        echo($orig == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$i]' src='static/cross.gif' alt='x' title='" . lang('Remove') . "' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">" : "");
        echo "\n";
    }
}
